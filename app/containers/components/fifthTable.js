...
import { Table, Button, Icon, Modal, Form, Input, Select, InputNumber, DatePicker, message } from 'antd'
import * as API from '../../apis'

const Option = Select.Option
const FormItem = Form.Item

const type = ['一号库（ห้อง1）', '二号库（ห้อง2）', '三号库（ห้อง3）', '四号库（ห้องแช่คว้านเม็ด）']
...
@Form.create()
export default class FifthTable extends React.Component{
  constructor() {
    super()
    this.state = {
      visible: false,
  ...
  async componentDidMount() {
    const res = await API.fifth.getStorage()
  ...
  async handleOk() {
    ...
    const res = await API.fifth.saveStorage(data)
    if(res.data) {
      message.success('保存成功！')
      window.location.reload()
      window.location.href = '#/loggedin?type=6'
    } else {
